...
def main() -> None:
        ...
        sample.append(
            {
                ...
                "images_type": (
                    type(p.get("images")).__name__ if isinstance(p, dict) else None
                ),
                "images_len": len(p.get("images") or []) if isinstance(p, dict) else 0,
                "childPosts_len": (
                    len(p.get("childPosts") or []) if isinstance(p, dict) else 0
                ),
            }
        )
        if p.get("childPosts"):
            first_child = p["childPosts"][0] if p["childPosts"] else {}
            sample[-1]["first_child_keys"] = (
                list(first_child.keys()) if isinstance(first_child, dict) else []
            )
            sample[-1]["first_child_displayUrl"] = (
                first_child.get("displayUrl") if isinstance(first_child, dict) else None
            )
    ...
    with open(raw_path, "w", encoding="utf-8") as f:
        json.dump(
            [
                {k: v for k, v in p.items() if k not in ("latestComments", "coauthors")}
                for p in posts[:2]
            ],
            f,
            ensure_ascii=False,
